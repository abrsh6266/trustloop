...
function computeAgreementStatus(
...
export async function createAgreement(
  ...
  if (!obligations.length) {
    throw new AppError(400, "Add at least one obligation to create an agreement.");
  }
  ...
  await prisma.$transaction(async (tx) => {
    ...
    if (trustDelta !== 0) {
      await tx.user.update({
        ...
        data: {
          trustScore: Math.max(0, obligation.assignedTo.trustScore + trustDelta),
        },
      });
    }
    ...
    await tx.activityLog.create({
      data: {
        ...
        userId: actorUserId,
        action:
          nextStatus === "DONE" ? ActivityAction.COMPLETED : ActivityAction.FAILED,
